-- Fix referral system setup - handles existing policies gracefully
-- Run this in your Supabase SQL editor

-- Add referral columns to profiles table (IF NOT EXISTS)
DO $$ 
BEGIN
    -- Add referral_code column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'referral_code') THEN
        ALTER TABLE public.profiles ADD COLUMN referral_code VARCHAR(10) UNIQUE;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'total_referrals') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'profiles' AND column_name = 'total_referral_earnings') THEN
        ALTER TABLE public.profiles ADD COLUMN total_referral_earnings NUMERIC(12,2) DEFAULT 0;
    END IF;
END $$;
...
-- Create trigger to auto-generate referral codes (drop first to avoid conflicts)
...
BEGIN
    ...
    RAISE NOTICE 'Commission rate set to 10%%';
    RAISE NOTICE 'All existing users now have referral codes';
END $$;
